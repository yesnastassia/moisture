...
df<-mutate(df,TotalMoisture=(MoistureTopNoNA*WeightTopNoNA+MoistureBottom*WeightBottom+MoistureLiveNoNA*WeightLiveNoNA)/(WeightTopNoNA+WeightBottom+WeightLiveNoNA))
...
df<-mutate(df,TotalMoisture_nolive=(MoistureTopNoNA*WeightTopNoNA+MoistureBottom*WeightBottom)/(WeightTopNoNA+WeightBottom))

##make a column for moisture that is top+live
df<-mutate(df,typeMoisture_live=(MoistureTopNoNA*WeightTopNoNA+MoistureLiveNoNA*WeightLiveNoNA)/(WeightTopNoNA+WeightLiveNoNA))
...
ggplot(df, aes(x=TotalMoisture,fill=GrassType))+geom_histogram(position="identity",alpha=.2,bins=50)+
  xlab("Total Moisture (as proportion of dry weight)")+ylab("Count") + theme(text = element_text(size=16))
...
p1<-ggplot(df, aes(x=NDWI, y=TotalMoisture)) + geom_point()+theme(text = element_text(size=16))
...
library(ggbiplot)
ggbiplot(PCA, choices=c(2,4)) + geom_point()
...
df <- df %>% mutate(class = case_when(TotalMoisture<=.1 ~ 1,
                                ...
                                TotalMoisture>.7 ~ 8))
tree<-mvpart(class~PC1+PC2+PC3+PC4, method="class", df, xv="pick", xvmult=100)
...
##fit just each type separately? is it worse in the short grass bc the variation is even lower?
